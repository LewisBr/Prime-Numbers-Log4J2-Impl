...
/**
 * PrimeDelegate implementation using a Millers-Rabin Primality implementation.
 *
 * @documentation https://en.wikipedia.org/wiki/Miller%E2%80%93Rabin_primality_test
 */
public class MillerRabinImplementation extends PrimeDelegate {
    private static final Logger             LOGGER      = LogManager.getLogger(MillerRabinImplementation.class);
    ...
    public ResponseWrapper generate(int firstNumber, int secondNumber) {
        ...
        LOGGER.debug("MillerRabinImplementation#generate Starting processing on number range.");

        IntStream.rangeClosed(lowerValue, higherValue).forEach(i -> {
            if (isPrime(i)) {
                primesArray.add(i);
            }
        });
        ...
        LOGGER.debug("MillerRabinImplementation#generate Finished processing on number range.");
    ...
    /**
     * Millers-Rabin implementation of isPrime.  Given a number input will run through a primality test to determine with a certain level of confidence
     * that the given input is prime, or composite (not).
     *
     * @param candidate Integer value of the number you want to know is prime or not.
     * @return boolean of if the given candidate is prime or not.
     */
    @Override
